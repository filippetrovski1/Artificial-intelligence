...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        if nasoka == "istok":
            ...
            newy = y - 1
            new_dir = "jug"
        if self.validni(newx, newy):
            newd = self.nd(newx, newy, dots)
            slednopomestuvanje["ProdolzhiPravo"] = (newx, newy, new_dir, newd)  # new_dir ili nasoka ne menuva nish
        ...
        if self.validni(newx, newy):
            newd = self.nd(newx, newy, dots)
            slednopomestuvanje["ProdolzhiNazad"] = (newx, newy, new_dir, newd)
        ...
        if self.validni(newx, newy):
            newd = self.nd(newx, newy, dots)
            slednopomestuvanje["SvrtiLevo"] = (newx, newy, new_dir, newd)
        ...
        if self.validni(newx, newy):
            newd = self.nd(newx, newy, dots)
            slednopomestuvanje["SvrtiDesno"] = (newx, newy, new_dir, newd)
        return slednopomestuvanje

    def validni(self, x, y):
        if x < 0 or x > 9 or y < 0 or y > 9:  # dokolku se pojavi barem 1 nadvor od granici vrati false ili se naogjame na mesto kade sto ima precka
            return False
        elif (x, y) in self.precki:
            return False
        else:
            return True

    def nd(self, x, y, dots):
        newdots = list(dots)
        if (x, y) in newdots:
            newdots.remove((x, y))
            return tuple(newdots)
        return dots
...
if __name__ == '__main__':
    ...
    for i in range(0, dot):
        pozicija = [int(j) for j in input().split(',')]
        pozicija = tuple(pozicija)
        dots_coordinates.append(pozicija)

    dots_coordinates = tuple(dots_coordinates)

    precki = [(0, 6), (0, 8), (0, 9), (1, 2), (1, 3), (1, 4), (1, 9),
              (2, 9), (3, 6), (3, 9), (4, 1), (4, 5), (4, 6), (4, 7), (5, 1), (5, 6), (6, 0), (6, 1), (6, 2), (6, 9),
              (8, 1), (8, 4), (8, 7), (8, 8), (9, 4), (9, 7), (9, 8)]

    precki = [tuple(i) for i in precki]

    pacman = Pacman((covece_x, covece_y, nasoka, dots_coordinates), precki)

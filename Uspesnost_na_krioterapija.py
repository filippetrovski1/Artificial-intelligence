...
if __name__ == '__main__':
    ...
    c = 0
    for red in test_set:
        prediction = gaussian.predict([red[:-1]])
        if prediction[0] == red[-1]:
            c += 1

    print(c/len(test_set))#tocnost na modelot

    entry = [float(element) for element in input().split(' ')]
    print(int(gaussian.predict([entry])))#klasata na predviduvanjee
    print(gaussian.predict_proba([entry]))#verojatnostite za pripadnost vo klasite
...
class Diskovi(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(self.l):
            ...
            if (niza[i] == 0):
                continue
            if i + 1 < self.l and i + 1 >= 0 and niza[i] != 0 and niza[i + 1] == 0:
                niza1 = list(state)
                niza1[i + 1] = niza[i]
                niza1[i] = 0
                if niza1 != niza:
                    succ["D1: Disk " + str(niza[i])] = tuple(niza1)
            if i + 2 < self.l and i + 2 >= 0 and niza[i] != 0 and niza[i + 1] != 0 and niza[i + 2] == 0:
                niza1 = list(state)
                niza1[i + 2] = niza[i]
                niza1[i] = 0
                if niza1 != niza:
                    succ["D2: Disk " + str(niza[i])] = tuple(niza1)
            if i - 1 >= 0 and i - 1 < self.l and niza[i] != 0 and niza[i - 1] == 0:
                niza1 = list(state)
                niza1[i - 1] = niza[i]
                niza1[i] = 0
                if niza1 != niza:
                    succ["L1: Disk " + str(niza[i])] = tuple(niza1)

            if i - 2 >= 0 and i - 2 < self.l and niza[i] != 0 and niza[i - 1] != 0 and niza[i - 2] == 0:
                niza1 = list(state)
                niza1[i - 2] = niza[i]
                niza1[i] = 0
                if niza1 != niza:
                    succ["L2: Disk " + str(niza[i])] = tuple(niza1)
...
if __name__ == '__main__':
    ...
    for i in range(l - n):
        pocetna.append(0)
    pocetna1 = tuple(pocetna)
    krajna1 = tuple(krajna)
    disk = Diskovi(n, l, (pocetna1), (krajna1))
